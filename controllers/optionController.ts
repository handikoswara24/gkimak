...
const getAllOption = catchAsyncErrors(async (req: NextRequest) => {
  ...
  const option = await Option.find(query)
    .skip((page - 1) * numberPerPage)
    .limit(numberPerPage);
  const total = await Option.find(query).countDocuments();
...
const updateOption = catchAsyncErrors(
...
const deleteOption = catchAsyncErrors(
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    const option = await Option.findById(params.id);
...
export { addOption, getAllOption, updateOption, deleteOption };
